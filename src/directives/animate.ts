 ...
 *
 * In the second parameter, a name must be given which is used to build the class and animation name
 * listened for.
 *
 ...
 *
 *  ${animate(showFoo ? html`<div>Foo</div>` : nothing, { name: 'foo' })}
 * `
 * ```
 */
export const animate = directive((value: unknown, options: AnimateOptions) => (parentPart: Part) => {
  ...
  const entryName = `${options.name}-entry`;
  const exitName = `${options.name}-exit`;
